...
# to define the user token as a part of the header and send it in a post request
headers = {
    "X-USER-TOKEN": TOKEN
}
...
def post_data(headers: dict, day="YYYYMMDD"):
    if day == "YYYYMMDD":
        ...
        now = datetime.now()
        day = now.strftime("%Y%m%d")  # to format today's date into: yyyyMMdd
    post_request_body = {
        "date": day,
        "quantity": input(f"How many hours did you {HABIT_ID} today?")}
